    ...
    while (goOn){
        ...
        if(nmb_of_games <= count_games){
            ...
            if(!inDestroyMode){
                ...
                if (!targetLocked){
                    haveTarget = nextDestroyShot(size, field, x, y, last_x, last_y, destroyPhase, distance);
                    }
                else {
                    int base_x = hit1_x;
                    int base_y = hit1_y;
                    if (shootA){
                         phaseWanted = dirPhaseA;
                        }
                    else {
                         phaseWanted = dirPhaseB;
                        }
                    int dx[] = {-1, 1, 0, 0};
                    int dy[] = {0, 0, -1, 1};
                    ...
                    while(tmpDist <= size){
                        int tx = base_x + dx[phaseWanted]*tmpDist;
                        int ty = base_y + dy[phaseWanted]*tmpDist;
